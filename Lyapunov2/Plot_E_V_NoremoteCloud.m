function []=Plot_E_V_NoremoteCloud()
clear;
clc;
close all;
 V=1000000000;
 % vector_V=[ 0.2 0.4  0.50 1 2 3 4]*V;
  vector_V=[4]*V;  % used to debug the Zm and femstar
  bar_E_4_temp=zeros(size(vector_V));
itermax=10;
bar_E_4=0;
 figure(1)
 for iter=1:itermax
% [bar_E_1,~]=E_V_NRC(1,0.1,0.1,vector_V); 
% bar_E_1=bar_E_1+bar_E_1;
%  [bar_E_2,~]=E_V_NRC(1,0.5,0.5,vector_V);
%  bar_E_2=bar_E_2+bar_E_2;
%  [bar_E_3,~]=E_V_NRC(1,0.5,0.1,vector_V);
%  bar_E_3=bar_E_3+bar_E_3;
  [bar_E_4_temp(iter),~]=E_V_NRC(1,0.1,0.5,vector_V);
